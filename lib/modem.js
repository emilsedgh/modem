    ...
    modem.getMessages = function (callback) {
      this.execute('AT+CMGL=1', function (data) {
        ...
        .filter(function (line) { 
          return line.length !== 0 && (line.slice(0,1) !== '+' || line.indexOf('0791') > 0);
        })
        .map(function (line, key) {
          line = line.trim();
          if (line.indexOf('0791') > 0) {
            line = line.substring(line.indexOf('0791'));
          }
          line = modem.processReceivedPdu(line, key);
          if (line !== undefined && line.hasOwnProperty('udh')) {
            delete line.udh;
          }
          return line;
        })
